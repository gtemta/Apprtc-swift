//
//  AgentRTCVideoChatViewController.swift
//  Apprtc
//
//  Created by IMF on 2016/10/13.
...
class AgentRTCVideoChatViewController: UIViewController,RTCEAGLVideoViewDelegate,ARDAppClientDelegate {
    ...
    override func viewDidLoad() {
        ...
        let zoomGestureRecognizer:UITapGestureRecognizer=UITapGestureRecognizer(target: self, action:#selector(AgentRTCVideoChatViewController.zoomRemote) )
        zoomGestureRecognizer.numberOfTapsRequired=2
        self.view.addGestureRecognizer(zoomGestureRecognizer)
        self.remoteView?.delegate=self
        self.localView?.delegate=self
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(AgentRTCVideoChatViewController.orientationChanged(_:)), name: "UIDeviceOrientationDidChangeNotification", object: nil)
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        self.urlLabel?.text = "等待用戶進入房間後開始服務 用戶進入房間後本行將消失"
    }
    ...
    func getMyService(_theID:String){
        let request = NSMutableURLRequest(URL: NSURL(string: ipadress + "api/service/?agent=" + _theID + "&format=json")!)
    ...
    func appclient(client: ARDAppClient!, didRotateWithLocal localVideoTrack: RTCVideoTrack!, remoteVideoTrack: RTCVideoTrack!) {
        NSObject.cancelPreviousPerformRequestsWithTarget(self, selector: #selector(AgentRTCVideoChatViewController.updateUIForRotation), object: nil)
        // Hack for rotation to get the right video size
        self.performSelector(#selector(AgentRTCVideoChatViewController.updateUIForRotation), withObject: nil, afterDelay: 0.2)
    }
